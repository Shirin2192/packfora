    ...
    $(document).ready(function() {
        $('#maxmold_form').on('submit', function(e) {
            e.preventDefault();
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        Swal.fire({
                            ...
                            $('#maxmold_form')[0].reset();
                        });
                    } else if (response.errors) {
                        const fieldNames = ['name', 'email', 'phone', 'services',
                            'message'
                        ];
                        ...
                        $.each(response.errors, function(fieldName, errorMessage) {
                            if (fieldNames.includes(fieldName)) {
                                ...
                                if (field.length) {
                                    field.after(
                                        '<div class="error-message" style="color:red; font-size: 13px; margin-top: 4px;">' +
                                        errorMessage + '</div>');
                                }
                            }
                        });
